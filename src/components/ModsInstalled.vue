        ...
        <div v-if="$store.state.appData && $store.state.appData.modSources" class="flex flex-wrap gap-4">
          <template v-for="mod in filteredMods"
            ...
            <template v-if="mod.type === 'allInOne'">
              <div v-if="this.isInstalledMod(mod.sid)" class="border rounded flex flex-col justify-between gap-4 p-4 bg-gray-300 dark:bg-gray-700 min-w-[300px]" >
                <!-- Div haut-->
...
<script>
  export default {
    ...
    computed: {
      ...
      filteredMods() {
        return this.$store.getters.filteredMods(this.selectedCategory, this.selectedGameVersion, "ONLY_INSTALLED");
      },
    },
    mounted() {
